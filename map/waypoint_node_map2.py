...
class Waypoint_Move(Node):
    ...
    def __init__(self):
        ...
        self.state = "free"
        self.timer = None
        self.hit_point = None   # Punto donde se detecta el obstaculo
    ...
    def update_distance(self, msg: IrIntensityVector):
        ...
        print(f"\n Array de sensores = [{self.distance}]")
    ...
    def move_robot(self, waypoint):
        ...
        if self.state != "free":
            ...
            self.avoid_obstacle(waypoint, twist)
    ...
    def avoid_obstacle(self, waypoint, twist):

        # Aqui deberia averiguar si hay un camino posible hasta el waypoint
        if self.hit_point is None:
            # Si es la primera vez que detecta el obstáculo, registrar el punto de impacto
            self.hit_point = (self.posX, self.posY)
            self.distance_to_goal_at_hit = self.calculate_distance(self.hit_point, waypoint)
            print(f"Hit point registrado en: {self.hit_point} con distancia al objetivo: {self.distance_to_goal_at_hit}")


        

        
        min_distance = np.min(self.distance) 
        if min_distance < 9:
            ...
            if direction in [2, 3, 4]:
                ...
                if self.calculate_distance([self.posX, self.posY], waypoint) < self.distance_to_goal_at_hit:
                    print("Nuevo Hitpoint ########################")
                    self.hit_point = [self.posX, self.posY]
                    self.move2waypoint(waypoint, twist)
                else:
                    if np.min(self.distance) < 3.5:
                        twist.angular.z = 0.2
                        twist.linear.x = 0.05
                    else:
